...
let api = new _jsOip.DaemonApi();
const limit = 50;

const getArtifacts = async () => {
  let _ref = await api.getLatestArtifacts(limit),
  ...
  for (let artifact of artifacts) {
    ...
    console.log("-----"); //console.log(artifacts)
  }
};
...
const getExplorerBlockHeight = () => {
  return getJSON('http://network.flo.cash/api/getblockcount').then(function (response) {
    return response;
...
const getBlockHashFromId = id => {
  return rp('http://network.flo.cash/api/getblockhash?index=' + id).then(function (data) {
    return data;
  }).catch(function (err) {
...
const getRawTransaction = tx => {
  return getJSON('http://network.flo.cash/api/getrawtransaction?txid=' + tx + '&decrypt=1').then(function (response) {
    return response;
  }).catch(function (error) {
...
const getBlockData = blockhash => {
  return getJSON('http://network.flo.cash/api/getblock?hash=' + blockhash).then(function (response) {
    return response;
  }).catch(function (error) {
...
function getIncomingTransactions() {
  getJSON('http://network.flo.cash/ext/getaddress/F7Rr4zfGR7ZBR5ukSVdSkuKt5tuX5kmYdY').then(function (response) {
    // Search for vouts to my address
    for (let tx of response.last_txs) {
      if (tx.type == "vout") {
        // Get txcomment from address
        getJSON('http://network.flo.cash/api/getrawtransaction?txid=' + tx.addresses + '&decrypt=1').then(function (response) {
          if (!existsInDB("/donation/" + tx.addresses)) {
            // Add up amounts and make sure it's over 50
            let total = 0;

            for (let i = 0; i < response.vout.length; i++) {
              if (response.vout[i].address == "FMg9M7GPuUAGKvhWmgWjoqYtMqmckD4tRF") total += response.vout[i].value;
            }
...
function getLatestTransactions() {
  getJSON('http://network.flo.cash/ext/getlasttxs/10/100').then(function (response) {
...
async function getLatestBlocks() {
  ...
  while (dbBlockHeight < explorerHeight) {
    ...
    let transactions = block.tx; // Parse transactions in block

    for (let i = 0; i < transactions.length; i++) {
      let tx = await getRawTransaction(transactions[i]); // Tweet about every 100 blocks mined

      console.log("  Parsing tx " + tx.txid + " in block " + dbBlockHeight + " [" + blockhash + "]"); // Look for coinbase if we're in a block divisible by 100
